            ...
            for (int kk = m.x3s; kk < m.x3l; kk++){
                for (int jj = m.x2s; jj < m.x2l; jj++){
                    for (int ii = m.x1s; ii < m.x1l; ii++){
                        ...
                        double rho = m.cons(IDN, kk, jj, ii); // Adust * m.cons(IDN, kk, jj, ii) * (sqrt(upperlim) - sqrt(lowerlim)) / (0.5 * pow(m.GrainSizeList(ss), 3)) * m.GrainMassList(ss);
    ...
    for (int kk = m.x3s; kk < m.x3l; kk++){
        for (int jj = m.x2s; jj < m.x2l; jj++){
            for (int ii = m.x1s; ii < m.x1l; ii++){
                ...
                if (abs(m.x2v(jj) - M_PI / 2.) < M_PI / 8. && abs(m.x1v(ii) - m.x1v(m.x1s)) < m.x1v(m.x1s) / 10.){
                    m.cons(IDN, kk, jj, ii) = 5.0;
