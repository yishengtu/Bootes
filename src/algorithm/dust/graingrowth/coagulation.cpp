...
void grain_growth(mesh &m, double dt){
    ...
    // #pragma acc parallel loop gang worker num_gangs (1024) collapse (3) vector_length(32) default (present) firstprivate(dt) \
    ...
    #pragma acc parallel loop gang worker num_gangs (1024) collapse (3) vector_length(32) default (present) firstprivate(dt) \
        ...
        num_here[0:NUMSPECIES], Mmat[0:NUMSPECIES])
    #else
	#pragma omp parallel for collapse (3) schedule(dynamic) private (grain_number_array, grain_vr_array, grain_vtheta_array, grain_vphi_array, num_here, Mmat)
    #endif
