            ...
            for (int jj = -x2excess; jj < m.nx2 + x2excess; jj++){
                for (int ii = -x1excess; ii < m.nx1 + x1excess; ii++){
                    double dx_axis, a;
                        ...
                        if      (axis == 0) { dx_axis = m.dx1(ii+x1excess); a = max(m.dprim(specIND, IV1, m.x3s + kk, m.x2s + jj, m.x1s + ii), - m.dprim(specIND, IV1, m.x3s + kk, m.x2s + jj, m.x1s + ii));}
                        else if (axis == 1) { dx_axis = m.dx2(jj+x2excess); a = max(m.dprim(specIND, IV2, m.x3s + kk, m.x2s + jj, m.x1s + ii), - m.dprim(specIND, IV2, m.x3s + kk, m.x2s + jj, m.x1s + ii));}
                        else                { dx_axis = m.dx3(kk+x3excess); a = max(m.dprim(specIND, IV3, m.x3s + kk, m.x2s + jj, m.x1s + ii), - m.dprim(specIND, IV3, m.x3s + kk, m.x2s + jj, m.x1s + ii));}
                    #elif defined(SPHERICAL_POLAR_COORD)
                        if      (axis == 0) {
                            dx_axis = m.dx1p(kk + x3excess, jj + x2excess, ii+x1excess);
                            a = max(m.dprim(specIND, IV1, m.x3s + kk, m.x2s + jj, m.x1s + ii), - m.dprim(specIND, IV1, m.x3s + kk, m.x2s + jj, m.x1s + ii));
                        }
                        else if (axis == 1) {
                            dx_axis = m.dx2p(kk + x3excess, jj + x2excess, ii+x1excess);
                            a = max(m.dprim(specIND, IV2, m.x3s + kk, m.x2s + jj, m.x1s + ii), - m.dprim(specIND, IV2, m.x3s + kk, m.x2s + jj, m.x1s + ii));
                        }
                        else                {
                            dx_axis = m.dx3p(kk + x3excess, jj + x2excess, ii+x1excess);
                            a = max(m.dprim(specIND, IV3, m.x3s + kk, m.x2s + jj, m.x1s + ii), - m.dprim(specIND, IV3, m.x3s + kk, m.x2s + jj, m.x1s + ii));
                        }
                    #else
                    ...
                    /** dconservatives **/
                    double BrhoL, BrhoR;
                    minmod(m.dcons(specIND, IDN, m.x3s + kk + x3excess, m.x2s + jj + x2excess, m.x1s + ii + x1excess),
                           ...
                           BrhoR);
                    double Bm1L, Bm1R;
                    minmod(m.dcons(specIND, IM1, m.x3s + kk + x3excess, m.x2s + jj + x2excess, m.x1s + ii + x1excess),
                           ...
                           Bm1R);
                    double Bm2L, Bm2R;
                    minmod(m.dcons(specIND, IM2, m.x3s + kk + x3excess, m.x2s + jj + x2excess, m.x1s + ii + x1excess),
                           ...
                           Bm2R);
                    double Bm3L, Bm3R;
                    minmod(m.dcons(specIND, IM3, m.x3s + kk + x3excess, m.x2s + jj + x2excess, m.x1s + ii + x1excess),
